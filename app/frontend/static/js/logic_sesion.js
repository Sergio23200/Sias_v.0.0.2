...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("loginForm").addEventListener("submit", async function (event) {
      ...
      if (tipoDocumento === "seleccionar") {
          document.getElementById("tipo_documento_incorrecto").textContent = "Debe seleccionar un tipo de documento";
          valid = false;
      } else {
      ...
      if (!/^\d{10}$/.test(numeroDocumento)) {
          document.getElementById("documentoIncorrecto").textContent = "Número de documento inválido (debe tener 10 dígitos)";
          valid = false;
      } else {
      ...
      if (contraseña.length < 6 || contraseña.length > 12) {
          document.getElementById("contraseñaIncorrecta").textContent = "La contraseña debe tener entre 6 y 12 caracteres";
          valid = false;
      } else {
      ...
      if (!rememberMe.checked) {
          document.getElementById("falta_terminos_y_condiciones").textContent = "Debe aceptar términos y condiciones para continuar";
          valid = false;
      } else {
...
// La variable "numero_documento" debe colocarse en el esquema de validación del backend para garantizar su existencia en el modelo de login.
